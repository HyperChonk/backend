...
export class VeBalService {
    ...
    async syncVeBalBalances() {
        ...
        if (networkContext.isMainnet) {
            ...
            if (networkContext.data.veBal?.delegationProxy) {
                ...
                // Log a warning if veBAL is not configured for the chain
                console.warn(
                    `veBAL delegation proxy not configured for chain ${networkContext.chain}. Skipping veBAL balance sync for L2 holders.`,
                );
            }
        }
