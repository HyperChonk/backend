...
export class NetworkContextService {
    ...
    public get config(): NetworkConfig {
        ...
        if (chainId && AllNetworkConfigs[chainId]) {
            return AllNetworkConfigs[chainId];
        }

        if (this.defaultChainId && AllNetworkConfigs[this.defaultChainId]) {
            return AllNetworkConfigs[this.defaultChainId];
        }

        throw new Error(
            `Network configuration not found for chainId: ${chainId || 'undefined'}, defaultChainId: ${
