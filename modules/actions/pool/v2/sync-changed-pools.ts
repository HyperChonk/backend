...
export const syncChangedPools = async (
    ...
    let lastSyncBlock = await getLastSyncedBlock(chain, PrismaLastBlockSyncedCategory.POOLS);

    // Prevent syncing from block 0 or older than than 15 mins ago
    const blocksIn15Mins = config[chain].avgBlockSpeed * 15 * 60;
    const blocks15MinsAgo = Number(latestBlock) - blocksIn15Mins;
    if (lastSyncBlock < blocks15MinsAgo) {
        lastSyncBlock = blocks15MinsAgo;
    }
    ...
    for (let i = 0; i < numBatches; i++) {
        ...
        try {
            ...
            // Do not fail the entire job on intermittent RPC errors for one batch; log and continue
            console.warn(
                `getChangedPoolsV2 failed for block range [${from}, ${to}]: ${error?.message || error}. Continuing...`,
            );
        }
    }
