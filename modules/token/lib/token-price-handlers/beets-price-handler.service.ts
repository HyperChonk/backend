...
export class BeetsPriceHandlerService implements TokenPriceHandler {
    ...
    public async updatePricesForTokens(
        ...
        const stSPrice = await prisma.prismaTokenCurrentPrice.findUnique({
            where: {
        ...
        if (!stSPrice) {
            throw new Error(`BeetsPriceHandlerService: Could not find stS price for token ${this.stSAddress} on SONIC chain.`);
        }
