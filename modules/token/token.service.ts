...
import { updatePrices } from './lib/token-price-handlers/price-handler-helper';
...
export class TokenService {
    ...
    public async setManualTokenPrice(
        tokenAddress: string,
        chain: Chain,
        price: number,
        timestamp?: number,
        ...
        const token = await this.getToken(tokenAddress, chain);
        if (!token) {
            throw new Error(`Token ${tokenAddress} not found on chain ${chain}`);
        }

        // Use the existing updatePrices helper function
        await updatePrices(
            'manual',
            [
                {
        ...
        // Update dynamic data if it already exists (only update price, don't create new records)
        const existingDynamicData = await prisma.prismaTokenDynamicData.findUnique({
            where: {
                tokenAddress_chain: {
        ...
        if (existingDynamicData) {
            const dynamicDataOperation = prisma.prismaTokenDynamicData.update({
                where: {
                    tokenAddress_chain: {
                        tokenAddress: tokenAddress.toLowerCase(),
                        chain,
            ...
            await prismaBulkExecuteOperations([dynamicDataOperation]);
        }

        // Clear cache to ensure fresh data
        this.cache.del(`${TOKEN_PRICES_CACHE_KEY}:${chain}`);
    }
}
