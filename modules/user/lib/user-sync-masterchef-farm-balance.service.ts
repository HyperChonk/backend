...
export class UserSyncMasterchefFarmBalanceService implements UserStakedBalanceService {
    ...
    public async syncChangedStakedBalances(): Promise<void> {
        ...
        if (!status) {
            console.log(
                `Staked balance sync status not found for chain ${networkContext.chain}. Running initialization first.`,
            );
            await this.initStakedBalances(['MASTER_CHEF']);
            ...
            if (!status) {
                throw new Error('UserMasterchefFarmBalanceService: Failed to initialize staked balances');
            }
        }
