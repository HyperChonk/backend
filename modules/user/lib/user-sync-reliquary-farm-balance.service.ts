...
export class UserSyncReliquaryFarmBalanceService implements UserStakedBalanceService {
    ...
    public async syncChangedStakedBalances(): Promise<void> {
        let status = await prisma.prismaUserBalanceSyncStatus.findUnique({
            where: { type_chain: { type: 'RELIQUARY', chain: networkContext.chain } },
        });

        if (!status) {
            console.log(
                `Reliquary staked balance sync status not found for chain ${networkContext.chain}. Running initialization first.`,
            );
            await this.initStakedBalances(['RELIQUARY']);
            // After initialization, get the status again to continue with sync
            status = await prisma.prismaUserBalanceSyncStatus.findUnique({
                where: { type_chain: { type: 'RELIQUARY', chain: networkContext.chain } },
            });
            if (!status) {
                throw new Error('UserReliquaryFarmBalanceService: Failed to initialize staked balances');
            }
        }
